    ...
    def evaluate(self, batch_matrix_list, eval_data, is_test=False, idx=0):
        ...
        if is_test:
            try:
                ...
                for i, (hits, num_pos) in enumerate(zip(bool_rec_matrix, pos_len_list)):
                    dcg = 0.0
                    idcg = 0.0

                    for idx, is_hit in enumerate(hits[:max_k]):
                        if is_hit:
                            dcg += 1.0 / np.log2(idx + 2)

                    for idx in range(min(num_pos, max_k)):
                        idcg += 1.0 / np.log2(idx + 2)

                    ndcg = (dcg / idcg) if idcg > 0 else 0.0
                    user_scores.append(ndcg)

                save_name = f"{self.config['model']}_{self.config['dataset']}_ndcg_scores.npy"
                np.save(save_name, np.array(user_scores))

            except Exception as e:
                print(f"Error saving per-user scores: {e}")
